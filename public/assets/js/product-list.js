			...
			success:function(response){
				...
				$('.product-img').attr('src',response.imgUrl);
				 for (var i = 0; i < response.option.length; i++) {
				 	var result = result + '<div class="option-content">'+
												...
												'<div class="row app-row">'+
													'<div class="table-responsive">'+
														'<table class="table table-stripped">'+
															...
															'<tr><td>'+response.option[i].price.lot_no+'</td><td>'+response.option[i].price.batch_no+'</td><td>'+response.option[i].price.manufacture_data+'</td><td>'+response.option[i].price.purchase_date+'</td><td>'+response.option[i].price.expiry_date+'</td></tr>'+
														'</table>'+
													'</div>'+
												'</div>'+
											'</div>';
